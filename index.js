...
const BOOK_DETAIL_URL = "https://openlibrary.org/works/";
const AUTHOR_URL = "https://openlibrary.org/search/authors.json?q=";
const AUTHOR_DETAIL_URL = "https://openlibrary.org/authors/";
...
app.get("/book/bookSearch", async (req, res) => {
  ...
  const searchUrl = `${BOOK_URL}${bookName}&limit=20`;
  const options = {
    method: "GET",
    headers: {
...
app.get("/book/bookdetail", async (req, res) => {
  ...
  const bookDetailUrl = `${BOOK_DETAIL_URL}${bookId}.json`;
  const options = {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
...
app.get("/author/authordetail", async (req, res) => {
  ...
  const authorDetailUrl = `${AUTHOR_DETAIL_URL}${authorId}.json`;
  const authorWorkUrl = `${AUTHOR_DETAIL_URL}${authorId}/works.json?limit=10`;
  const options = {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
